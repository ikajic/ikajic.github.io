...
As happens to be the case with many such towns in that part of the world, its history is rooted in heavy mining activity that started some time in 19th century.
...
the morning sessions would be such a pleasant activity.
...
and software for simulation of neural networks, both for cognitive modelling and
...
which turned out to be a more robust (and colourful) solution.
...
spontaneously (such as re-arranging the talks so people could watch World Cup
...
I learned a lot while having a great time, and I would encourage anyone working in this domain to
